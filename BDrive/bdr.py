...
def login_handle():
    ...
    p=input('Password: ')
    bilibili = Bilibili()
    if bilibili.login(username=u, password=p):
...
while True:
    print('login,')
    ch = input('BiliDrive > ')
    if ch == 'login' : login_handle()
    if ch == 'upload' : upload_handle()
    if ch == 'uploadall' : uploadall_handle()
    if ch == 'download' : download_handle()
    if ch == 'downloadall' : downloadall_handle()
    if ch == 'tomd' : tomd_handle()
    if ch == 'info' : info_handle()
    if ch == 'history' :history_handle()
    if ch == 'exit' : exit()